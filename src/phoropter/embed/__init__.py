...
Phoropter never bakes in an embedding model. A corpus pins an embedder as
...
refused. Providers are registered under the ``phoropter.embedders``
entry-point group (see docs/adapters.md).

This module is dependency-free on purpose — the shipped HTTP providers
(:mod:`phoropter.embed.ollama`, :mod:`phoropter.embed.openai_compat`) live in
...
_PROBE_TEXT = "phoropter dimension probe"
...
class Embedder(ABC):
    ...
    :class:`~phoropter.errors.EmbedderError`.
    """
...
class EmbedderRegistry:
    ...
    :class:`~phoropter.errors.EmbedderError` listing what is registered.
    Third-party providers arrive either by explicit :meth:`register` calls or
    via :meth:`load_entry_points` (group ``phoropter.embedders``).
    """
    ...
    def load_entry_points(self) -> int:
        """Register every factory in the ``phoropter.embedders`` entry-point group.
        ...
        count = 0
        for ep in entry_points(group="phoropter.embedders"):
