...
Adapters are registered under the ``phoropter.stores`` entry-point group and
...
core phoropter modules (enforced by import-linter).
"""
...
from phoropter.model import RetrievedHit, Slice, SliceRef
...
class VectorStoreAdapter(ABC):
    ...
    - **Payload-complete hits.** Every hit and every fetched point carries a
      fully reconstructed :class:`~phoropter.model.Slice` — all coordinates,
    ...
    - **Config is frozen.** :meth:`create_corpus` persists the
      ...
      (:class:`~phoropter.errors.CorpusMismatchError`).
    - **Errors.** Unknown corpus → :class:`~phoropter.errors.CorpusNotFoundError`;
      duplicate creation → :class:`~phoropter.errors.CorpusExistsError`; unknown
      document → :class:`~phoropter.errors.DocumentNotFoundError`; backend
      unreachable or failing → :class:`~phoropter.errors.StoreError`.
    """
    ...
    async def verify_corpus(self, corpus: str) -> list[str]:
        ...
        :class:`~phoropter.errors.CorpusNotFoundError` instead.
        """
    ...
    async def upsert_slices(
        ...
        (:class:`~phoropter.errors.CorpusMismatchError`), as are vectors of the
    ...
    async def get_document_meta(self, corpus: str, document_id: str) -> DocumentRecord:
        """The registry entry, or :class:`~phoropter.errors.DocumentNotFoundError`."""
    ...
    async def delete_document(self, corpus: str, document_id: str) -> None:
        ...
        Raises :class:`~phoropter.errors.DocumentNotFoundError` if the document
        is not registered.
        """
...
def available_store_names() -> tuple[str, ...]:
    """Adapter names registered under the ``phoropter.stores`` entry-point group, sorted."""
    from importlib.metadata import entry_points

    return tuple(sorted({ep.name for ep in entry_points(group="phoropter.stores")}))


def load_store_class(name: str) -> type[VectorStoreAdapter]:
    """Resolve a store adapter class from the ``phoropter.stores`` entry-point group."""
    from importlib.metadata import entry_points

    for ep in entry_points(group="phoropter.stores"):
        if ep.name == name:
            cls = ep.load()
            if not (isinstance(cls, type) and issubclass(cls, VectorStoreAdapter)):
                raise StoreError(
                    f"entry point {name!r} in group 'phoropter.stores' is not a "
