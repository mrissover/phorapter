...
- ``{prefix}__meta`` — one point per corpus holding its frozen
  :class:`~phoropter.stores.CorpusConfig`;
...
:func:`~phoropter.stores.pack_markers`, and document-level metadata lives on
...
from phoropter.ids import PHOROPTER_NAMESPACE
...
_INSTALL_HINT = 'QdrantStore requires the qdrant extra: pip install "phoropter[qdrant]"'
...
def _registry_uuid(kind: str, *parts: str) -> _uuid.UUID:
    ...
    name = "|".join(["phoropter-registry", kind, *(_escape(p) for p in parts)])
    return _uuid.uuid5(PHOROPTER_NAMESPACE, name)
...
class QdrantStore(VectorStoreAdapter):
    """:class:`~phoropter.stores.VectorStoreAdapter` over ``qdrant-client``'s async client.

    ``prefix`` namespaces every collection this store touches, so multiple
    phoropter deployments (or a test run) can share one Qdrant instance
